...
function loadGridSwan(){
    $("#jqGrid").jqGrid({
        url: baseURL + 'sys/sifanyobj/list',
        datatype: "json",
        colModel: [			
			{ label: 'id', name: 'id', index: 'id', width: 50, key: true },
			{ label: '所属类id', name: 'classId', index: 'class_id', width: 80 }, 			
			{ label: '对象名', name: 'name', index: 'name', width: 80 }, 			
...
$(function () {
    ...
    $.get(baseURL + "sys/sifanyobj/select", function(r){
        ztreeMain = $.fn.zTree.init($("#objTree"), setting1, r.objEntityLists);
        var node = ztreeMain.getNodeByParam("id", vm.sifanyObj.id);
	...
	methods: {
        ...
        getSifanyclass: function(){
            ...
            $.get(baseURL + "sys/sifanyclass/select", function(r){
                ...
                var node = ztree.getNodeByParam("id", vm.sifanyObj.classId);
                ztree.selectNode(node);
                var a = JSON.stringify(r.classLists);
                // vm.sifanyClass.className = node.name;
            })
        },
		add: function(){
			vm.showList = false;
		...
		saveOrUpdate: function (event) {
		    $('#btnSaveOrUpdate').button('loading').delay(1000).queue(function() {
                var url = vm.sifanyObj.id == null ? "sys/sifanyobj/save" : "sys/sifanyobj/update";
                $.ajax({
                    type: "POST",
                    url: baseURL + url,
		...
		del: function (event) {
            ...
            layer.confirm('确定要删除选中的记录？', {
               ...
               if(!lock) {
		            ...
		            $.ajax({
                        type: "POST",
                        url: baseURL + "sys/sifanyobj/delete",
                        contentType: "application/json",
        ...
        classTree: function(){
            layer.open({
                ...
                btn1: function (index) {
                    ...
                    //选择父类
                    vm.sifanyObj.classId = node[0].id;
                    vm.sifanyObj.className = node[0].name;
        ...
        showscene: function (event) {
            ...
            $("#swan-svg").attr('src', '/swan-admin/statics/gojs/samples/show.html?extensions=');
